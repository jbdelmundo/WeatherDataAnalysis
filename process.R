...
script.dir <- "C:/Users/gaagu/Documents/School/CS 297 Spatio-Temporal Analysis/Project/Climate Data"
setwd(script.dir)
...
min_date <- as.Date("1973-01-01", "%Y-%m-%d")
...
# Plot NAs per row
png("NA_plots.png")
plot(data.pcp24$date, row_nas_full, type="l", col="red", ylim=c(0,100), ylab="# of NAs per day", xlab="date")
lines(data.pcp24$date, row_nas, type="l", col="blue")
legend("topright", c("full data", "subset data"), col=c("red", "blue"), lty=1)
dev.off()
...
# Plot stations in a map
png("map_of_stations.png")
newmap <- getMap(resolution = "low")
plot(newmap, xlim = c(114, 127), ylim = c(5, 21), asp = 1)
points(sum_pcp24$LON, sum_pcp24$LAT, col="red")
points(sum_pcp24.sub$LON, sum_pcp24.sub$LAT, col="blue")
dev.off()

# temp.data <- mice(data.pcp24.sub[,-1],m=5,maxit=50,meth='pmm',seed=500)

# for (colnum in seq(2,ncol(data.pcp24))) {
#   png(paste0(colnames(data.pcp24)[colnum],".png"))
#   plot(data.pcp24$date, data.pcp24[,colnum], ylab=colnames(data.pcp24)[colnum], type="l", col="red", xlab="date")
#   dev.off()
# }
